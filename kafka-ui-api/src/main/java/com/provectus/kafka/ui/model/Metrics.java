...
public class Metrics {
  ...
  public static Metrics empty() {
    return Metrics.builder().brokerBytesInPerSec(Map.of()).brokerBytesOutPerSec(Map.of())
        .topicBytesInPerSec(Map.of()).topicBytesOutPerSec(Map.of()).perBrokerMetrics(Map.of())
        .messageInMeanRate(Map.of()).messageInOneMinuteRate(Map.of()).messageInFiveMinuteRate(Map.of())
        .messageInFifteenMinuteRate(Map.of()).fetchRequestsMeanRate(Map.of())
        .fetchRequestsOneMinuteRate(Map.of()).fetchRequestsFiveMinuteRate(Map.of())
        .fetchRequestsFifteenMinuteRate(Map.of()).produceRequestsMeanRate(Map.of())
        .produceRequestsOneMinuteRate(Map.of()).produceRequestsFiveMinuteRate(Map.of())
        .produceRequestsFifteenMinuteRate(Map.of()).build();
  }

  public Stream<RawMetric> getSummarizedMetrics() {
    return perBrokerMetrics.values().stream().flatMap(Collection::stream)
        .collect(
            toMap(RawMetric::identityKey, m -> m, (m1, m2) -> m1.copyWithValue(m1.value().add(m2.value()))))
        .values().stream();
  }
