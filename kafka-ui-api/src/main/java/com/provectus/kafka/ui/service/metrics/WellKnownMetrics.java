...
class WellKnownMetrics {
  ...
  // BrokerTopicMetrics TotalFetchRequestsPerSec
  ...
  // BrokerTopicMetrics TotalProduceRequestsPerSec
  ...
  private void updateBrokerIOrates(Node node, RawMetric rawMetric) {
    ...
    if (!brokerBytesInFifteenMinuteRate.containsKey(node.id()) && rawMetric.labels().size() == 1
        && "BytesInPerSec".equalsIgnoreCase(rawMetric.labels().get("name"))
        && containsIgnoreCase(name, "BrokerTopicMetrics") && endsWithIgnoreCase(name, "FifteenMinuteRate")) {
      brokerBytesInFifteenMinuteRate.put(node.id(), rawMetric.value());
    }
    if (!brokerBytesOutFifteenMinuteRate.containsKey(node.id()) && rawMetric.labels().size() == 1
  ...
  private void updateTopicsIOrates(RawMetric rawMetric) {
    ...
    if (topic != null && containsIgnoreCase(name, "BrokerTopicMetrics")) {
      if (endsWithIgnoreCase(name, "MeanRate")) {
      ...
      } else if (endsWithIgnoreCase(name, "OneMinuteRate")) {
        String nameProperty = rawMetric.labels().get("name");
        if ("MessagesInPerSec".equalsIgnoreCase(nameProperty)) {
      ...
      } else if (endsWithIgnoreCase(name, "FiveMinuteRate")) {
        String nameProperty = rawMetric.labels().get("name");
        if ("MessagesInPerSec".equalsIgnoreCase(nameProperty)) {
      ...
      } else if (endsWithIgnoreCase(name, "FifteenMinuteRate")) {
        String nameProperty = rawMetric.labels().get("name");
